...
export function useBalance(group: Group, expenses: Expense[]) {
  return useMemo(() => {
    ...
    const netBalances = computeNetBalances(expenses, phones, nameMap);
...
export function useMyBalance(group: Group, expenses: Expense[], myPhone: string): number {
